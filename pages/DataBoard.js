...
function DataBoard() {
    ...
    const boardId = 2; // 자료게시판

    // 상태 변수
    const [dataBoardList, setDataBoardList] = useState([]); // 자료게시판 목록
    ...
    const getDataBoardList = async () => {
        try {
            ...
            setDataBoardList(res.data.boards);
    ...
    // 등록 버튼 눌렀을 때 자료게시글 등록 페이지로 이동
    const goAddDataBoard = () => {
            ...
            <div className="px-4 md:px-10 py-2 md:py-7">
                <div className="flex items-center justify-center">
                    <p tabIndex="0" style={{fontSize: "xx-large"}}
                       className="p-4 rounded-full focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl leading-normal text-gray-800">자료게시판</p>
                </div>
                        ...
                        <div
                            ...
                            <div className="mr-2">
                                <select id="Com"
                                        ...
                                        className="cursor-pointer text-base text-gray-800 outline-none border-gray-100 shadow-md px-4 py-2 rounded-lg w-24">
                            ...
                            <input onChange={searchWordOnChangeHandler} value={searchWord}
                                className="text-base text-gray-400 flex-grow outline-none px-2 rounded-lg border-gray-100 shadow-md"
                                   type="text" placeholder="검색어를 입력하세요."/>
                            <div className="ms:flex items-center px-2 rounded-lg space-x-4 mx-auto ">
                                <button onClick={()=>{getDataBoardList();}} type="button"
                                    className="bg-purple-300 text-white text-base rounded-lg px-4 py-2 font-thin shadow-md">검색
                                </button>
                            </div>
                        </div>
                    </form>
                    <button onClick={goAddDataBoard}
                            className="bg-purple-300 text-white text-base rounded-lg px-4 py-2 font-thin shadow-md">등록
                    </button>
                </div>
                        ...
                        <tr tabIndex="0" className="focus:outline-none  border border-gray-100 shadow-md" style={{height: '3.5rem'}}>
                            ...
                            <td className="pl-5">
                                <div className="flex items-center">
                                    ...
                                    <p className="text-base leading-none text-gray-700 ml-2">작성자</p>
                                </div>
                            </td>
                            <td className="pl-5">
                            </td>
                            <td className="pl-5">
                                <div className="flex items-center">
                                    <p className="text-base leading-none text-gray-700 ml-2">조회수</p>
                                </div>
                            </td>
                            <td className="pl-5">
                            </td>
                                ...
                                return (
                                    <>
                                        ...
                                        <tr tabIndex="0"
                                            ...
                                            <td className="focus:text-indigo-600 ">
                                                <div className="flex items-center pl-12">
                                                    <Link to={`/app/board-detail?boardId=${boardId}&postId=${v.postId}&userId=${v.userId}`}
                                ...
                                <li>
                                    <button onClick={() => {
                                        setCurrentPage(currentPage - 1)
                                            ...
                                            className="h-10 px-3 text-gray-600 transition-colors duration-150 rounded-l-lg focus:shadow-outline hover:bg-purple-100">
                                        ...
                                        return pagesToShow.map(page => (
                                            <li key={page}>
                                                <button
                                                    ...
                                                    className={`h-10 px-5 transition-colors duration-150 focus:shadow-outline ${currentPage === page ? 'bg-purple-500 text-white border border-purple-500' : 'text-gray-600 hover:bg-purple-100'}`}>
                                                    {page}
                                                </button>
                                            </li>
                                        ));
                            ...
                            <li>
                                <button onClick={() => {
                                    setCurrentPage(currentPage + 1)
                                        ...
                                        className="h-10 px-3 text-gray-600 transition-colors duration-150 bg-white rounded-r-lg focus:shadow-outline hover:bg-purple-100">
                                    <svg className="w-4 h-4 fill-current" viewBox="0 0 20 20">
                                        <path
